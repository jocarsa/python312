...
def mostrarInsertar(marco,marcolistado):
    marco.pack(padx=50,pady=50)
    marcolistado.pack_forget()

def mostrarListado(marco,marcolistado):
...
def miMenu(ventana,marco,marcolistado):
    ...
    operaciones.add_command(label="Listado",command=lambda:mostrarListado(marco,marcolistado))
    operaciones.add_command(label="Insertar",command=lambda:mostrarInsertar(marco,marcolistado))
...
def configurarVentana(ventana):
    ...
    ventana.geometry("1200x600+20+20")
    ventana.iconbitmap("../../jv.ico")

def marcoInsertar(marco,marcolistado):
...
def marcoListado(marco,marcolistado):
